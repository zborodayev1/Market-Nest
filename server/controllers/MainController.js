...
const updateSetting = async (req, res, field, value) => {
    try {
        ...
        res.json({ message: `${field.charAt(0).toUpperCase() + field.slice(1)} changed successfully` });
    } catch (err) {
        console.error(err);
...
const validateField = (res, fieldName, fieldValue) => {
    if (!fieldValue) {
        res.status(400).json({ message: `${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)} is required` });
        return false;
    }
    return true;
};
